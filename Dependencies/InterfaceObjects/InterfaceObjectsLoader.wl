...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated InterfaceObjects loader file *)


Temp`PackageScope`InterfaceObjectsLoading`Private`$DependencyLoad=
  TrueQ@Temp`PackageScope`InterfaceObjectsLoading`Private`$DependencyLoad;
If[Temp`PackageScope`InterfaceObjectsLoading`Private`$DependencyLoad,
  Unprotect["`InterfaceObjects`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["`InterfaceObjects`PackageScope`Private`Constants`$TopLevelLoad"]]=False,
  Unprotect["InterfaceObjects`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["InterfaceObjects`PackageScope`Private`Constants`$TopLevelLoad"]]=
    MemberQ[$ContextPath, "Global`"]
  ];


If[Temp`PackageScope`InterfaceObjectsLoading`Private`$DependencyLoad,
  Evaluate[Symbol["`InterfaceObjects`PackageScope`Private`$RootContext"]]=$Context;
  BeginPackage["`InterfaceObjects`"],
  Evaluate[Symbol["InterfaceObjects`PackageScope`Private`$RootContext"]]="";
  BeginPackage["InterfaceObjects`"]
  ]


ClearAll[InterfaceObjects];
InterfaceObjects::usage="InterfaceObjects is a manager head for the InterfaceObjects package";
...
InterfaceObjects["Directory"]:=
  $PackageDirectory;
$PackageDirectory=
  DirectoryName@$InputFileName;


InterfaceObjects["Name"]:=
  $PackageName;
$PackageName=
  "InterfaceObjects";
...
InterfaceObjects["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
InterfaceObjects["Mode"]:=$PackageLoadingMode
$PackageLoadingMode=
...
InterfaceObjects["Dependencies"]:=$PackageDependencies;
$PackageDependencies=
...
InterfaceObjects["DependencyBase"]:=$PackageDependencyBase;
...
InterfaceObjects["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
InterfaceObjects["Contexts"]:=$PackageContexts;
  ...
  $PackageContexts=
    Join[
      If[$PackageLoadingMode==="Dependency",
        ...
        {
          "InterfaceObjects`",
          "InterfaceObjects`PackageScope`Private`",
          "InterfaceObjects`PackageScope`Package`"
          },
...
InterfaceObjects["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
InterfaceObjects["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
InterfaceObjects["FileContexts"]:=$PackageFileContexts;
...
InterfaceObjects["DeclaredPackages"]:=$DeclaredPackages;
...
InterfaceObjects["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,InterfaceObjects],"`"]
    ];
  ...
  With[{
    newcont=
      If[StringStartsQ[context, "`"],
        "InterfaceObjects"<>context<>scope<>"`",
        context<>scope<>"`"
        ],
...
InterfaceObjects::nodep="Couldn't load dependency `` of type ``";
InterfaceObjects::nodup="Couldn't update dependency `` of type ``";
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[InterfaceObjects::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[InterfaceObjects::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[InterfaceObjects::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
$PackageErrorMessage=
  "InterfaceObjects encountered exception ``";
...
PackageThrowMessage[
  ...
  PackageThrowMessage[
    MessageName[InterfaceObjects, tag],
    body,
    ops
    ];
PackageThrowMessage[
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[InterfaceObjects, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageFailureException[
  ...
  (
    Set[MessageName[InterfaceObjects, tag], body];
    PackageFailureException[
      MessageName[InterfaceObjects, tag],
      body,
      ops
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[InterfaceObjects, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[InterfaceObjects],
          Nest[
            Insert[#, _, {1, 1, 1}]&,
            (HoldPattern[Verbatim[HoldPattern]][
              InterfaceObjects[s_String, ___]
              ]:>_),
            n-1
            ]:>s,
          Infinity
...
PackagePrepPackageSymbol[]:=
  Switch[$AllowPackageSymbolDefinitions,
    None,
      If[Length@OwnValues[InterfaceObjects]==0,
        Remove[InterfaceObjects],
        DownValues[InterfaceObjects]={}
        ],
    False,
      If[Length@OwnValues[InterfaceObjects]==0,
        Clear[InterfaceObjects],
        DownValues[InterfaceObjects]={}
        ],
    _,
      PackageAttachMainAutocomplete[]
...
If[Temp`PackageScope`InterfaceObjectsLoading`Private`$DependencyLoad,
  If[
    (Clear@`InterfaceObjects`PackageScope`Private`$loadAbort;!#)&@
      `InterfaceObjects`PackageScope`Private`$loadAbort,
    Unprotect[`InterfaceObjects`PackageScope`Private`$LoadCompleted];
    `InterfaceObjects`PackageScope`Private`$LoadCompleted=True;
    `InterfaceObjects`PackageScope`Private`PackagePostProcessContextPathReassign[]
    ],
  If[
    (Clear@InterfaceObjects`PackageScope`Private`$loadAbort;!#)&@
      InterfaceObjects`PackageScope`Private`$loadAbort,
...
Remove["Temp`PackageScope`InterfaceObjectsLoading`Private`*"]