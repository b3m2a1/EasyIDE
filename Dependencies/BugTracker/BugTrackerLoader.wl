...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated BugTracker loader file *)


Temp`PackageScope`BugTrackerLoading`Private`$DependencyLoad=
  TrueQ@Temp`PackageScope`BugTrackerLoading`Private`$DependencyLoad;
If[Temp`PackageScope`BugTrackerLoading`Private`$DependencyLoad,
  Unprotect["`BugTracker`PackageScope`Private`$TopLevelLoad"];
  Evaluate[Symbol["`BugTracker`PackageScope`Private`$TopLevelLoad"]]=False,
  Unprotect["BugTracker`PackageScope`Private`$TopLevelLoad"];
  Evaluate[Symbol["BugTracker`PackageScope`BugTrackerLoading`$TopLevelLoad"]]=
    MemberQ[$ContextPath, "Global`"]
  ];


If[Temp`PackageScope`BugTrackerLoading`Private`$DependencyLoad,
  Evaluate[Symbol["`BugTracker`PackageScope`Private`$RootContext"]]=$Context;
  BeginPackage["`BugTracker`"],
  Evaluate[Symbol["BugTracker`PackageScope`Private`$RootContext"]]="";
  BeginPackage["BugTracker`"]
  ]


ClearAll[BugTracker];
BugTracker::usage="BugTracker is a manager head for the BugTracker package";
...
BugTracker["Mode"]:=$PackageLoadingMode
$PackageLoadingMode=
...
BugTracker["Dependencies"]:=$PackageDependencies;
$PackageDependencies=
...
BugTracker["DependencyBase"]:=$PackageDependencyBase;
...
BugTracker["AllowRescoping"]:=$AllowPackageRescoping;
$AllowPackageRescoping=
  Replace[
    Lookup[$PackageLoadSpecs, "AllowRescoping"],
    Except[True|False]->$TopLevelLoad
    ];
BugTracker["AllowRecoloring"]:=$AllowPackageRecoloring;
$AllowPackageRecoloring=
  Replace[
    Lookup[$PackageLoadSpecs, "AllowRecoloring"],
    Except[True|False]->$TopLevelLoad
    ];
BugTracker["AllowAutocompletions"]:=$AllowPackageAutocompletions;
...
BugTracker["FileContexts"]:=$PackageFileContexts;
...
BugTracker["DeclaredPackages"]:=$DeclaredPackages;
...
BugTracker["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,BugTracker],"`"]
    ];
...
BugTracker::nodep="Couldn't load dependency `` of type ``";
BugTracker::nodup="Couldn't update dependency `` of type ``";
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[BugTracker::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[BugTracker::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[BugTracker::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
$PackageErrorMessage=
  "BugTracker encountered exception ``";
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[BugTracker, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageFailureException[
  ...
  (
    Set[MessageName[BugTracker, tag], body];
    PackageFailureException[
      MessageName[BugTracker, tag],
      body,
      ops
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[BugTracker, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[BugTracker],
          Nest[
            Insert[#, _, {1, 1, 1}]&,
            (HoldPattern[Verbatim[HoldPattern]][
              BugTracker[s_String, ___]
              ]:>_),
            n-1
            ]:>s,
          Infinity
...
If[Temp`PackageScope`BugTrackerLoading`Private`$DependencyLoad,
  If[
    (Clear@`BugTracker`PackageScope`Private`$loadAbort;!#)&@
      `BugTracker`PackageScope`Private`$loadAbort,
    Unprotect[`BugTracker`PackageScope`Private`$LoadCompleted];
    `BugTracker`PackageScope`Private`$LoadCompleted=True;
    `BugTracker`PackageScope`Private`PackagePostProcessContextPathReassign[]
    ],
  If[
    (Clear@BugTracker`PackageScope`Private`$loadAbort;!#)&@
      BugTracker`PackageScope`Private`$loadAbort,
...
Remove["Temp`PackageScope`BugTrackerLoading`Private`*"]
