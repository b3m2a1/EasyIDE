...
If[Temp`PackageScope`EmsLoading`Private`$DependencyLoad,
  Evaluate[Symbol["`Ems`PackageScope`Private`$RootContext"]]=$Context;
  BeginPackage["`Ems`"],
  Evaluate[Symbol["Ems`PackageScope`Private`$RootContext"]]="";
  BeginPackage["Ems`"]
  ]
...
Ems["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
Ems["Dependencies"]:=$PackageDependencies;
$PackageDependencies=
...
Ems["DependencyBase"]:=$PackageDependencyBase;
...
Ems["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
...
Ems["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
Ems["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
Ems["FileContexts"]:=$PackageFileContexts;
...
Ems["DeclaredPackages"]:=$DeclaredPackages;
...
Ems["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,Ems],"`"]
    ];
...
Ems::nodep="Couldn't load dependency `` of type ``";
Ems::nodup="Couldn't update dependency `` of type ``";
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[Ems, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
If[Temp`PackageScope`EmsLoading`Private`$DependencyLoad,
  If[
    (Clear@`Ems`PackageScope`Private`$loadAbort;!#)&@
      `Ems`PackageScope`Private`$loadAbort,
    Unprotect[`Ems`PackageScope`Private`$LoadCompleted];
    `Ems`PackageScope`Private`$LoadCompleted=True;
    `Ems`PackageScope`Private`PackagePostProcessContextPathReassign[]
    ],
  If[
    (Clear@Ems`PackageScope`Private`$loadAbort;!#)&@
      Ems`PackageScope`Private`$loadAbort,
...
Remove["Temp`PackageScope`EmsLoading`Private`*"]