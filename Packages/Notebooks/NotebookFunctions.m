...
getStylesheetStylesheet[nbFile_]:=
  (Global`wtf = getNbFileSSheet[nbFile])/.
    loadSheetStyles[]
...
getFileStylesheet[nb_, file_]:=
  Module[
    ...
    mainName = 
      ...
      Replace[main, 
        {
          FrontEnd`FileName[_, fn_]:>
            StringSplit[fn, "."|"-"][[1]]
          }
        ];
    cleanStylesheetName[mainName,
...
ideNotebookPutFile[nb_NotebookObject, f_String]:=
  Module[
    ...
    aug = CurrentValue[nb, AutoGeneratedPackage];
    If[aug === Automatic,
...
ideNotebookLoadPluginsMenu//Clear
ideNotebookLoadPluginsMenu[nb_]:=
        ...
        Replace[state,
          Hold[s_]:>
            Button[
              ...
              If[!AssociationQ[s], s=<||>];
              If[Length[s]>0,
                destroyMenu[Dynamic[s]],
                makeMenu[Dynamic[s], EvaluationBox[], 
...
createDynamicTabRow[refresh_]:=
  With[
    ...
    Dynamic[
      refresh;
      Pane[
        Row@MapThread[createTabObject, tns], {Automatic, 35}, Alignment->Bottom],
      TrackedSymbols:>{refresh}
      ]/.cv->CurrentValue
    ]
...
createTabObject[tabName_String, file_]:=
  Style[
    Panel[
      Row@{
          Spacer[15],
  ...
  Module[{cell, tools, tags},
    ...
    tools =
      GridBox[
        ReplacePart[
          Map[{#, ""}&, tools],
          {1, 2}->ButtonBox["", BaseStyle->"ToolbarCloseButton", ButtonData->tag]
          ], 
