...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated EasyIDE loader file *)


Temp`PackageScope`EasyIDELoading`Private`$DependencyLoad=
  TrueQ@Temp`PackageScope`EasyIDELoading`Private`$DependencyLoad;
If[Temp`PackageScope`EasyIDELoading`Private`$DependencyLoad,
  Unprotect["`EasyIDE`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["`EasyIDE`PackageScope`Private`Constants`$TopLevelLoad"]]=False,
  Unprotect["EasyIDE`PackageScope`Private`Constants`$TopLevelLoad"];
  Evaluate[Symbol["EasyIDE`PackageScope`Private`Constants`$TopLevelLoad"]]=
    MemberQ[$ContextPath, "Global`"]
  ];


If[Temp`PackageScope`EasyIDELoading`Private`$DependencyLoad,
  Evaluate[Symbol["`EasyIDE`PackageScope`Private`$RootContext"]]=$Context;
  BeginPackage["`EasyIDE`"],
  Evaluate[Symbol["EasyIDE`PackageScope`Private`$RootContext"]]="";
  BeginPackage["EasyIDE`"]
  ]
...
EasyIDE["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
EasyIDE["Mode"]:=$PackageLoadingMode
$PackageLoadingMode=
...
EasyIDE["Dependencies"]:=$PackageDependencies;
$PackageDependencies=
...
EasyIDE["DependencyBase"]:=$PackageDependencyBase;
...
EasyIDE["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
EasyIDE["Contexts"]:=$PackageContexts;
  ...
  $PackageContexts=
    Join[
      If[$PackageLoadingMode==="Dependency",
        ...
        {
          "EasyIDE`",
          "EasyIDE`PackageScope`Private`",
          "EasyIDE`PackageScope`Package`"
          },
...
EasyIDE["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
EasyIDE["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
EasyIDE["FileContexts"]:=$PackageFileContexts;
...
EasyIDE["DeclaredPackages"]:=$DeclaredPackages;
...
EasyIDE["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,EasyIDE],"`"]
    ];
...
EasyIDE::nodep="Couldn't load dependency `` of type ``";
EasyIDE::nodup="Couldn't update dependency `` of type ``";
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[EasyIDE::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[EasyIDE::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[EasyIDE::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[EasyIDE, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageFailureException[
  ...
  (
    Set[MessageName[EasyIDE, tag], body];
    PackageFailureException[
      MessageName[EasyIDE, tag],
      body,
      ops
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[EasyIDE, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[EasyIDE],
...
If[Temp`PackageScope`EasyIDELoading`Private`$DependencyLoad,
  If[
    (Clear@`EasyIDE`PackageScope`Private`$loadAbort;!#)&@
      `EasyIDE`PackageScope`Private`$loadAbort,
    Unprotect[`EasyIDE`PackageScope`Private`$LoadCompleted];
    `EasyIDE`PackageScope`Private`$LoadCompleted=True;
    `EasyIDE`PackageScope`Private`PackagePostProcessContextPathReassign[]
    ],
  If[
    (Clear@EasyIDE`PackageScope`Private`$loadAbort;!#)&@
      EasyIDE`PackageScope`Private`$loadAbort,
...
Remove["Temp`PackageScope`EasyIDELoading`Private`*"]
